...
import { InexToken } from './interfaces/inex.interface';
import { Options as HttpOptions, request } from './utils/httpClient';

dayjs.extend(utc);

export interface InexSDKBaseOptions {
    publicKey?: string;
    privateKey?: string;
...
export class InexSDKBase {
    protected options: Required<InexSDKBaseOptions> =
        {} as Required<InexSDKBaseOptions>;

    constructor(options?: Partial<InexSDKBaseOptions>) {
        if (!options) {
            return;
        }
        this.setOptions(options);
    }
    protected setOptions(options: Partial<InexSDKBaseOptions> = {}) {
    ...
    protected auth(
        ...
        if (this.options.publicKey && this.options.privateKey) {
            ...
            return this.request<InexToken>(`${path}`, {
                method: 'POST',
                json: data,
            });
        } else {
