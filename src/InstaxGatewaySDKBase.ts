...
import { InstaxGatewayToken } from './interfaces/instaxgateway.interface';
import { Options as HttpOptions, request } from './utils/httpClient';

dayjs.extend(utc);

export interface InstaxGatewaySDKBaseOptions {
    publicKey?: string;
    privateKey?: string;
...
export class InstaxGatewaySDKBase {
    protected options: Required<InstaxGatewaySDKBaseOptions> =
        {} as Required<InstaxGatewaySDKBaseOptions>;

    constructor(options?: Partial<InstaxGatewaySDKBaseOptions>) {
        if (!options) {
            return;
        }
        this.setOptions(options);
    }
    protected setOptions(options: Partial<InstaxGatewaySDKBaseOptions> = {}) {
    ...
    protected auth(
        ...
        if (this.options.publicKey && this.options.privateKey) {
            ...
            return this.request<InstaxGatewayToken>(`${path}`, {
                method: 'POST',
                json: data,
            });
        } else {
